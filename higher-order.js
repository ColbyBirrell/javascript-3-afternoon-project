...
//Code Here
let evenNumbers = mixedNumbers.filter(number => {
  //filter keeps the values and will assign to the new variable
  if (number % 2 === 0) {
    return true;
  } else {
    return false;
...
//Code Here
let postTaxPrices = prices.map(price => {
  return price * 1.07;
});
...
//Code Here
let totalPopulation = populations.reduce((sum, population) => {
